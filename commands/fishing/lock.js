...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('lock')
        .setDescription('Allows you to lock a fish to prevent the sale of it.')
        .addStringOption(option =>
            option.setName('identifier')
                .setDescription('The identifier of the fish you want to lock.')
                .setRequired(true)),
    async execute(interaction) {
        ...
        if (fishData.locked) return interaction.reply({
            embeds: [
                new MessageEmbed()
                .setColor('#FC0000')
                .setTitle("<:yukinon:839338263214030859> Fish already locked")
                .setDescription(`The fish \`${fishData.fishId}\` · \`${fishData.type}\` is already locked.`)
            ]
        })
        await FishData.updateOne({ fishId: identifier }, { $set: { locked: true } });
        return interaction.reply({
            embeds: [
                new MessageEmbed()
                .setColor('#03fc84')
                .setTitle("Fish successfully locked")
                .setDescription(`The fish \`${fishData.fishId}\` · \`${fishData.type}\` has been locked.`)
                .setFooter({text: `By locking a fish, you are preventing it from being sold. To unlock it, use the /unlock command.`})
            ]
        })
    }
}
